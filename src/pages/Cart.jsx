...
import { clearBasket, removeCartBook,plusCartBook,minusCartBook } from '../redux/actions/cart';
...
function Cart() {
    ...
    const books = Object.keys(items).map(obj => { return items[obj].items[0] });
    ...
    const onPlusBook = (id) => {
        dispatch(plusCartBook(id))
    }

    const onMinusBook = (id) => {
        dispatch(minusCartBook(id));
    }
            ...
            <div className="container container--cart">
                {totalCount === 0 ? (
                    <div className="cart cart--empty">
                        ...
                        <p>
                            Вероятней всего, вы не выбрали ещё книгу.<br />
              Для того, чтобы выбрать книгу, перейди на главную страницу.
            </p>
                    ...
                    <div className="content__items">
                        {books && books.map(book => <CartItem {...book} key={`${book.name}_${book.id}`} booksAmount={items[book.id] && items[book.id].items.length} totalPrice={items[book.id].totalPrice} onRemoveBook={onRemoveBook} onPlusBook={onPlusBook} onMinusBook={onMinusBook}/>)}
                    </div>
